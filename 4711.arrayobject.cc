...
public:
   ...
   DECLARE_CONOBJECT(Array);
	...
	U32					getIndexFromValue(StringTableEntry value);
	U32					getIndexFromKey(StringTableEntry key);
	...
	U32			getIndexFromKeyValue( StringTableEntry key, StringTableEntry value);
...
//	Searches array for the first matching value from the current array position
// Will return -1 if no matching index found
...
//	Searches array for the first matching key from the current array position
// Will return -1 if no matching index found
U32 Array::getIndexFromKey( StringTableEntry key)
{
	U32 foundIndex = -1;
	for(U32 i=mCurrentIndex; i<mArray.size(); i++)
	{
...
//	Searches array for the first matching key and value from the current array position
// Will return -1 if no matching index found
U32 Array::getIndexFromKeyValue( StringTableEntry key, StringTableEntry value)
{
	U32 foundIndex = -1;
	for(U32 i=mCurrentIndex; i<mArray.size(); i++)
	{
...
void Array::moveIndex(U32 prev, U32 index)
{
	...
	mArray[index].value = NULL;
	mArray[index].key = NULL;
}
...
void Array::sort(bool valsort, bool desc, bool numeric)
{
	...
	Con::printf("sort is numeric=%d, ascending=%d, value=%d",(S32)numeric,(S32)sIncreasing,(S32)valsort);
